...
def train_catboost(dataset12, dataset3):
    ...
    train_dmatrix =dataset12_x
    label=dataset12.label
    predict_dmatrix =dataset3_x# lgb.Dataset(dataset3_x)

    # catboost模型训练
    
    cbt=CatBoostClassifier(loss_function="Logloss",
                           eval_metric="AUC",
                           task_type="GPU",
                           learning_rate=0.01,
                           iterations=70000,
                           ...
                           border_count=64,
                           train_dir=path+r'/catboosttrain/')

    # 使用xgb.cv优化num_boost_round参数
    #cvresult = cbt.cv(params, train_dmatrix,label,num_boost_round=1000, nfold=5, stratified=False, shuffle=True, metrics='auc',seed=0,callbacks=callbacks)
    #num_round_best = len(cvresult['valid auc-mean'])
    #print('Best round num:', len(cvresult['valid auc-mean']))
    #print('Best round num: ', num_round_best)

    # 使用优化后的num_boost_round参数训练模型
    #watchlist = [(train_dmatrix, 'train')]
    model = cbt.fit( train_dmatrix,label)#, evals=watchlist)

    model.save_model(path+'/train_dir/catboostmodel')
    #params['predictor'] = 'cpu_predictor'
    #model = cbt.Booster(model_file=path+'/train_dir/catboostmodel')
    #model.load_model(path+'/train_dir/catboostmodel')
    ...
    dataset3_predict.to_csv(path+'/result/result_finally_catboost_preds.csv', index=None, header=None)
...
if __name__ == '__main__':
    ...
    # grid_search_xgb()
    train_catboost(dataset12, dataset3)
